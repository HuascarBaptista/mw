...
APP_PATH4 = File.expand_path( APP_ROOT_CUSTOM  + 'config/application', APP_ROOT_CUSTOM  + 'bin/rails')
...
begin
	...
	class DemoDaemon < Daemon
		...
		def service_main(*args)
			...
			while running?
				...
				begin
					...
					File.open(LOG_FILE,'a+'){ |f| f.puts "Error en FotosDeteccion #{Time.now} err=#{err}"}
					raise
				end
			end
		end 
		...
		def service_stop(*args)
			File.open(LOG_SERVICIOS_FILE, "a"){ |f| f.puts "Servicio FotosDeteccion detenido #{Time.now}" }
			exit! 
		end


	 def checkImages
	 	#Se detecta cual es el servidor almacenado en el MW
		ftpp = Ftp.all.first
		if !ftpp.present?
		...
		camaras.each do |camara|
			...
			#Se conecta con dicho servidor y se procede a listar todas las imágenes almacenadas en las carpetas de cada cámara
			ftp = objetoFtp(ftpp,camara.ip)
			unless ftp.present?
				File.open(LOG_FILE,'a+'){ |f| f.puts "Servicio FTP no encendido #{Time.now}"}
				return
			end
			...
			files.each do |file|
				...
				begin
					...
					#Se descarga la imagen del FTP al MW
					...
					#Enviar la foto a todos los scripts autenticados con esa cámara
					...
					autenticacion_camaras.each do |autenticacion|
						...
						camara_ip =camara.ip
						#Se especifica la direccion URL donde se encuentra la imagen descargada
						...
						threads << Thread.new do
							#Por cada llamada a los scripts se crea un hilo con un tiempo de espera de 5 segundos para no saturar el MW
							sleep 5
							begin
								...
								begin
									#Si no dio algun error al enviar la imagen se procede a marcarla como ya usada
	...
	def objetoFtp(ftpp,ipCamara)
		...
		begin
			...
			File.open(LOG_FILE,'a+'){ |f| f.puts "Error seteando FTP #{Time.now} #{e}" }
			return false
		end
		if ftpp.path[-1,1]!="/"
			ftpp.path+="/"
